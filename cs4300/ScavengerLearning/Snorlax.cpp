...
namespace bjs
{
  namespace Scavenger
  {
    ...
    double Snorlax::GetTotals(std::string attr, int origin){
        double total = 0;
        if(attr == "color"){
...
//            std::cout << "GETTING COUNTS FOR TABLES: SIZE2 : " << total << std::endl;
    ...
    std::vector<double> Snorlax::ProbOriginGivenColorShapeSize(std::string size, std::string color, std::string shape){
...
//        double size2_G_origin = mSizes2[size2][origin].second;
        ...
        double denominator = denom + denom2 + denom3;
...
//        mProblemRecharge->SetRocks(1);
    ...
    std::pair<int, int> Snorlax::Coordinates(int x, int y){
        std::pair<int, int> coordinates;
        coordinates.first = x;
        coordinates.second = y;
        return coordinates;
    }
    Cell * Snorlax::getCell(std::pair<int, int> pair){
        Cell * c =  new Cell();
        if(start){
            start = false;
            startstate->SetX(0);
    ...
    std::map<std::string, std::string> Snorlax::ParseObject(std::string object) {
        ...
        pair.first = "";
        pair.second = 1; // Laplace smoothing ------
...
//                                std::cout << objid << std::endl;
            ...
            if (isspace(object[i]) && !c && obj){
                ...
                // IF NOT EXISTS, ADD THE ORIGIN PAIRS WITH LAPLACE SMOOTHING
                if(it == mColors.end())
                {
                    mColors[color] = vec;
                }
...
void Snorlax::GetObjectCounts(std::map<std::string, std::string> parsed_object, std::string val) {
        ...
        std::pair<std::string, int> pair;
        pair.first = "";
        pair.second = 1; // LAPLACE SMOOTHIN -----------<
        vec.push_back(pair);
        vec.push_back(pair);
        vec.push_back(pair);
        //IF A NEW ORIGIN
...
//        else std::cout << " IT SEEMS THIS OBJECT WAS PREVIOUSLY DEPOSITED.  POSSIBLY IN THE FIRST 100.  SKIP IT-----------------------------------------. " << std::endl << val << "=value " << " ID = " << mDropoffObjectIds[gbestindex] << std::endl;
}

std::string Snorlax::GetObjectValue(const ai::Agent::Percept * percept) {
     ai::Agent::PerceptAtom a = percept->GetAtom("DEPOSIT_VALUE");
    std::string value = a.GetValue().c_str();
//    std::cout << "VALUE AT PERCEPT = " << value << " " << mDropoffObjectIds[dropoffIndex] << std::endl;
    bool obj1 = true;
    std::string val = "";
 ...
 void Snorlax::PrintQuit(std::string message){
    std::map<std::string, std::vector<std::pair<std::string, int>>>::const_iterator it;
    ...
    std::cout <<"THERE WERE a total of " << gtotaleverything << " objects and " << gTotalDeposits << " were deposited and " << negs << " were bad deposits." << std::endl;
...
//                    std::cout << " these are the object ids: " << object_id << std::endl;
    ...
    std::pair<std::string, double> Snorlax::DepositorNot(){
        double lastgreatest = 0;
        std::string lastbestid = "";
        std::pair<std::string, double> p;
        for(unsigned int i = 0; i < mDropoffObjectIds.size(); i++){
            if(mDropoffObjectIds[i] == gSEEN) continue;
            std::map<std::string, std::map<std::string, std::string>>::const_iterator it;
            it = mParsedObjects.find(mDropoffObjectIds[i]);
            std::map<std::string, std::string> parsed_object;
            ...
            double ev = probs[0] * 50;
            double ev1  = probs[1] * 10;
            double ev2 = probs[2] * -50; 
            ev = ev+ev1+ev2;
            if(ev > lastgreatest) {
                lastgreatest = ev; 
                lastbestid = mDropoffObjectIds[i]; 
                gbestindex = i; 
                p.first = mDropoffObjectIds[i]; 
                p.second = ev;
//                std::cout << lastbestid << " " << lastgreatest << " " << ev << std::endl;
            }
            
        }
//        std::cout << "THIS IS THE RESULT: " << p.first << " " << p.second << std::endl;
        return p;
    }
...
//                        std::cout << " THE return object is : " << mDropoffObjectIds[gbestindex]+" "+mObjects[mDropoffObjectIds[gbestindex]] << std::endl;
                        checkDepositValue = false;
                        ...
                        if(first100index < 100){
                            ...
                            mDropoffObjectIds[first100randomindex] = gSEEN; 
                        }
//                        else it = mParsedObjects.find(mDropoffObjectIds[dropoffIndex]);
                        else it = mParsedObjects.find(mDropoffObjectIds[gbestindex]);
                            GetObjectCounts(it->second, val);
                        if(first100index > 99){ std::cout << " THE ACTUAL VALUE WAS: " << val << " AND THE NUMBER OF DEPOSITS THUS FAR IS: " << gTotalDeposits << std::endl << std::endl;
                            dropoffIndex++; 
                            mDropoffObjectIds[gbestindex] = gSEEN;
                        }
                        else first100index++;
                        gTotalDeposits++;
                        continue;
                    }
                    else if(!done && (end/* || (int)mDropoffObjectIds.size() > 1500*/) && needDropOff && (dropoffIndex+100) < (int)mDropoffObjectIds.size() && mCharge_num > 1){
//                        if(!first100){
...
//                                    std::cout << randomnums[i] << " " << first100randomindex << std::endl;
                        ...
                        std::pair<std::string, double> p = DepositorNot();
//                        std::map<std::string, std::map<std::string, std::string>>::const_iterator it;
...
//                        double ev1  = probs[1] * 10;
//                        double ev2 = probs[2] * -50; 
//                      std::cout << " THESE ARE THE PROBABILITIES OF EACH ORIGIN OF THIS OBJECT: " << floor(probs[0]*1000)/10 << "% " << floor(probs[1]*1000)/10 << "% " << floor(probs[2]*1000)/10 << "%" << std::endl
//                                << " THESE ARE THE ORIGIN TABLES:                               " << floor((mOriginTable[0]/origintotal)*1000)/10 << "% " << floor((mOriginTable[1]/origintotal)*1000)/10 << "% " << floor((mOriginTable[2]/origintotal)*1000)/10 << "%" << std::endl
//                                << " THESE ARE ESTIMATED VALUES WHATEVER THESE ARE:             " << floor(ev*1000)/10 << "% " << floor(ev1*1000)/10 << "% " << floor(ev2*1000)/10 << "%   AND EV: " << ev + ev1 + ev2 << "pts" << std::endl << std::endl;
//                        if(ev > ev2 || ev1 > ev2){
                        if(p.second > 20){
//                            std::cout << "I THINK I WILL DEPOSIT THIS ITEM " << std::endl;
                            action->SetObjectId(p.first);
...
//                            std::cout << dropoffIndex << " " << mDropoffObjectIds.size() << std::endl;
                            dropoffIndex++;
                            continue;
                        }
...
//                cit = mLearnedObjects.find(parsed_object["OBJECT"]);
//                if(cit != mLearnedObjects.end())
//                {                                    
...
//                            mExhaustionMode = 1;
                ...
                if(recharge_search->Search()){
                    ...
                    std::cout << "DROPOFF SIZE ============ " << mDropoffObjectIds.size() << std::endl;
