  ...
  namespace Scavenger
  {
    ...
    std::vector<ai::Search::Action *> Problem::Actions(const ai::Search::State * const state_in)
    {
      std::vector<ai::Search::Action *> actions;
          ...
          bool noquit = 0;
//          int passableterrain = 0; // should be more than one unless possibly the home cell.
          if(state->GetCharge() < 0.0 || state->GetHP() < 0.0){
              std::cout << "NOT ENOUGH CHARGE OR HP FOR THIS ACTION" << std::endl;
                 return actions;
          }
          Cell * mCell = state->GetCell();
//          bool bypass = false;
//          if((state->GetX() == 1000 && state->GetY() == 0) || (state->GetX() == -1000 && state->GetY() == 0) ||(state->GetX() == 0 && state->GetY() == 1000) || (state->GetX() == 0 && state->GetY() == -1000)) bypass = true;
//          if(state->GetX() == 0 && state-> GetY() == 0) passableterrain = 1;
          if(!mRocks){
...
//                     passableterrain += 1;
    ...
    ai::Search::State *Problem::Result(const ai::Search::State * const state_in, const ai::Search::Action * const action_in)
    {
      ...
      new_state->SetY(state->GetX());
      new_state->SetZ(state->GetZ());
      std::cout << "Is it ever not in there" << state->GetX() << " " << state->GetX() << std::endl;
      return new_state;
    }
